...
hip_confirm_or_abort "Rollback HIP in development container '${HIP_CONTAINER_NAME}'?"
...
BACKUP_PATH="$(hip_latest_backup_for_target development)"
if [ -z "${BACKUP_PATH}" ]; then
  hip_log_error "no development backup found in /config/.storage/hip/backups"
  exit 1
fi
