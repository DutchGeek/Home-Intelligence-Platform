...
if [ "${SHOW_VERSION}" = "true" ]; then
	hip_print_version_info production
	exit 0
fi
...
if [ "${HIP_DRY_RUN}" = "true" ]; then
	hip_print_dry_run_plan production
	exit 0
fi

hip_progress 2 10 "Checking git status policy"
hip_git_check_target_policy production

hip_progress 3 10 "Pulling latest changes"
hip_git_pull_if_enabled

hip_progress 4 10 "Requesting deployment confirmation"
hip_confirm_or_abort "Deploy HIP to production container '${HIP_CONTAINER_NAME}' from '${HIP_REPO_ROOT}'?"
...
BACKUP_PATH="$(hip_backup_current production "${VERSION}" "${TIMESTAMP}")"
